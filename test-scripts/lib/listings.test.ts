import { Listing, ListingsStatusEnum } from '../api/Api';
...
describe('Get All Listings', () => {
  test('Successful Run', async () => {
    ...
    const listingsInterface = new ListingsInterface(user, password, url, passkey);
...
describe('get closed listings', () => {
  test('successfully gets closed listings', async () => {
    const url = 'https://doorway';
    const passkey = 'testkey';
    const user = 'good.user@gooduser.com';
    const password = 'goodpassword';
    let li = new ListingsInterface(user, password, url, passkey);
    const closedListings = await li.getClosedListings();
    expect(closedListings.length).toBe(63);
    for (let listing of closedListings) {
      expect(listing.status).toBe(ListingsStatusEnum.Closed);
    }
  });
});
